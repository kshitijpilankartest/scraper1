...
export const Renderer = {
  renderStats(articles = []) {
    ...
    const newCount   = safeArticles.filter(a => a.is_new).length;
    const savedCount = safeArticles.filter(a => a.is_saved).length;
    const sources    = [...new Set(safeArticles.map(a => a.source))].length;
  ...
  renderArticles(articles = [], filter = 'all', searchQuery = '') {
    ...
    const safeArticles = Array.isArray(articles) ? articles : [];
    console.log(`[Renderer] Pipeline Start: ${safeArticles.length} articles`);

    // STRICT FILTER: Exclude any article without an image
    let filtered = safeArticles.filter(a => !!a.thumbnailUrl);
    console.log(`[Renderer] After Image Filter: ${filtered.length} articles`);

    const imageFilterRemovedAll = safeArticles.length > 0 && filtered.length === 0;
    ...
    if (filtered.length === 0) {
      if (empty) {
        ...
        if (imageFilterRemovedAll) {
          if (emptyTitle) emptyTitle.textContent = 'No articles with images';
          if (emptyDesc) emptyDesc.textContent = 'We found articles, but none have valid thumbnails at the moment.';
        } else {
          if (emptyTitle) emptyTitle.textContent = 'No articles found';
          if (emptyDesc) emptyDesc.textContent = 'Try a different filter or hit Refresh to pull the latest.';
        }
      }
      return;
    }
