...
async function boot() {
  console.log('[Scarper] Starting boot sequence...');
  try {
    ...
    console.log(`[Scarper] Loaded ${allArticles.length} cached articles.`);
    ...
    if (Storage.shouldFetch()) {
      console.log('[Scarper] 24h limit reached or no data. Triggering fetch...');
      await runFetch();
    } else {
      console.log('[Scarper] Articles are fresh. Skipping auto-fetch.');
    }

    // 3. Schedule next auto-refresh at midnight-ish (check every hour)
    scheduleAutoRefresh();
  } catch (err) {
    console.error('[Scarper] Critical error during boot:', err);
    Renderer.showToast('Initialization failed. Check console.', 'error');
  } finally {
    // Always clear the "Initializing..." state by ensuring lastUpdated is set
    // and loading overlay is hidden.
    const lastFetched = Storage.getLastFetched();
    Renderer.setLastUpdated(lastFetched);
...
async function runFetch() {
  ...
  try {
    console.log('[Scarper] Fetching feeds via Proxy...');
    const { articles, errors } = await Fetcher.fetchAll((done, total) => {
      Renderer.updateProgress(done, total);
    });

    console.log(`[Scarper] Raw Fetch Result: ${articles.length} articles`);
    ...
    if (articles.length > 0) {
      ...
      console.log(`[Scarper] New: ${newlyFetched.length}, Needs Thumbnail: ${needsThumbnail.length}`);
      ...
      console.log(`[Scarper] Storage updated. Total cached: ${allArticles.length}`);
      ...
      console.warn('[Scarper] Fetched 0 articles. Falling back to cache.');
      Renderer.showToast('Could not fetch new articles. Showing cached.', 'warn');
      // Still render stats even if fetch failed to clear "---"
      Renderer.renderStats(allArticles);
      Renderer.renderArticles(allArticles, currentFilter, searchQuery);
    }
    ...
    // Ensure stats are rendered even on failure
    Renderer.renderStats(allArticles);
    Renderer.renderArticles(allArticles, currentFilter, searchQuery);
  } finally {
    Renderer.setLoading(false);
